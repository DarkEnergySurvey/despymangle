...
######################################################################
def make_syste_masks(keyword, config, Nmolys, Image_tab, molyids, exclude=None, magic_num=0, keys=['SUM', 'MIN', 'MAX', 'MEDIAN', 'MEAN', 'WMEAN']):
...
def make_csv_ccdgon(config, fn_csv, Image_tab, log):
    ...
    for i in range(0, idxs.shape[0]):
        ...
        print >>g, linefmt % (ccdgon_id, red_image_filename[idx], coadd_filename,
                              amp, 1./weights[idx], config['pfw_attempt_id'], config['band'])
    g.close()
...
def make_csv_molygon(config, fn_csv, molyids, log, Image_tab, Nmolys):
    ...
    for i in range(0, len(molyids)):
        #print i
        print >>g, linefmt % (molyids[i], coadd_filename, coaddtile_id, band, num_images[i], 
