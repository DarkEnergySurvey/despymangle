...
def make_comp(coadd_fullname, fn_mg, fn_star, fn_bleed, plot_fullname,
    ...
    pylab.axis('scaled')
    pylab.pcolormesh(A[0], A[1], data2[tuple(M)].T,
                     vmin=np.min(data2[tuple(M)]),
                     vmax=np.max(data2[tuple(M)]))
    pylab.colorbar()
    ...
    pylab.axis([np.min(limitx), np.max(limitx), np.min(limity), np.max(limity)])
    pylab.axis('scaled')
    pylab.pcolormesh(A[0], A[1], (data2[tuple(M)].T-D),
                     vmin=-np.max(data2[tuple(M)])/5.,
                     vmax=np.max(data2[tuple(M)])/5.)
    pylab.colorbar()
    pylab.title('Difference (Coadd - Mangle)')

    ng = np.where(D == 0)
    rap = data2[tuple(M)].T/D
    rap[ng] = -1
